...
1. Loads agent configuration (name, model, instructions, temperature, top_p, tools) from [config/it_support_agent.yaml](config/it_support_agent.yaml).
...
|---|---|
| Core agent properties (name, model, description, instructions, temperature, top_p) | `configure_agent` + the YAML config |
...
Set the project endpoint and model deployment name (see the repo README):
...
By default the agent, conversation, vector store, and uploaded files are deleted at the end. To keep the agent so you can open it under **Prompt Agents** in the Foundry Toolkit for VS Code, set:

```powershell
$env:FOUNDRY_KEEP_AGENT = "1"
```
